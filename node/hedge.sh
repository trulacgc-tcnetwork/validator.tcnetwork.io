...
# Node
NODE_REPO=https://github.com/hedgeblock/hedge.git
NODE_VERSION=v0.1.0
NODE_REPO_FOLDER=
NODE_DAEMON=hedged
NODE_ID=berberis-1
...
NODE_GENESIS_FILE=https://rpc-berberis.hedgeblock.io/genesis
NODE_GENESIS_CHECKSUM=
NODE_ADDR_BOOK=false
NODE_ADDR_BOOK_FILE=

# Service
NODE_SERVICE_NAME=hedge
...
function installNode() {
  ...
  sudo wget https://github.com/hedgeblock/testnets/releases/download/v0.1.0/hedged_linux_amd64_v0.1.0
  sudo mv hedged_linux_amd64_v0.1.0 hedged

  sudo chmod +x hedged
  mv hedged $HOME/go/bin/

  sudo set -eux
  sudo wget -O /lib/libwasmvm.x86_64.so https://github.com/CosmWasm/wasmvm/releases/download/v1.3.0/libwasmvm.x86_64.so
  hedged version
...
function initNode() {
  ...
  if $NODE_GENESIS_ZIP; then
    ...
    curl $NODE_GENESIS_FILE? | jq ".result.genesis" >$HOME/$NODE_FOLDER/config/genesis.json
  fi
